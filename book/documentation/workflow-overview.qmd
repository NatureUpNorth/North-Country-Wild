...
Consider this. A single game camera on a single deployment can collect thousands of images. If you deploy several cameras, you will very quickly start to amass tens to hundreds of thousands of images, or more! A single deployment of an acoustic recorder might garner 45 gb of data! Before you know it, you've collected TB of data, and are on your way to a data management nightmare!
...
3.  How will you organize all the images and audio files so that you can:

- easily find files from any particular deployment?
- avoid confusing files between deployments?
- prepare images for classification?
- back up the files to the cloud?

4.  How will you classify the image files? What steps need to happen to prepare images for classification?
...
Our workflow for dealing with image data is different for our workflow for dealing with acoustic data, because there are extra steps required to prepare the images for upload to the [Zooniverse](https://www.zooniverse.org/projects/barthelmess/north-country-wild).

This manual is broken into two sections, one for camera trap images and the other for audio files from passive acoustic recorders.
...
Our current workflow (Summer 2026) involves using the Zooniverse platform for image classification by volunteers (rather than using machine learning to ID animals in images). As a result, we also have to process all of our images by downsizing them for upload to the Zooniverse. As we downsize them, we also add copyright information the image EXIF data and copy the images to a "Processed" folder with the same directory structure as the "Raw" folder.
...
Nested within each of these directories is a unique directory for each camera deployment. Each of those subfolders is named with a combination of the Camera number and SD card number that was used for each deployment. The overall organization therefore looks something like what is shown in @fig-1.2.
